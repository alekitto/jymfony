 ...
 */
const createRedirectResolver = (options, host, proxy, info, onProgress) => {
    let redirectHeaders = {};
    ...
    return async (location, context) => {
        ...
        let url;
        try {
            url = NativeHttpClient._parseUrl(location);
        } catch (e) {
        ...
        url = NativeHttpClient._resolveUrl(url, String(info.url));
        ...
        if (shouldResolve) {
            const resolved = await dnsResolve(host, context, info, onProgress);
            url.host = resolved.includes(':') ? '[' + host + ']' : host;
            url.port = null;
        }
...
export default class NativeHttpClient extends implementationOf(
    ...
    request(method, url, options = {}) {
        ...
        const resolver = async () => {
            ...
            return [ createRedirectResolver(options, host, proxy, info, onProgress), url ];
        };
